# animation5 for Repthon edit by ~ @E_7_V
...
@zq_lo.rep_cmd(pattern=f"تتحميل$")
async def _(event):
    if event.fwd_from:
        return
...
@zq_lo.rep_cmd(pattern=f"العد التنازلي$")
async def _(event):
    if event.fwd_from:
        return
    animation_interval = 1
